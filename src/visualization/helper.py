import torch
from typing import Any, Iterable, Tuple, Union, Optional, Dict
...
def show(
    ...
    for i in range(n_samples):
        ...
        if stats:
            ...
            anchored_text = AnchoredText(txt, loc=2, prop=dict(fontsize=10, color='purple'))
    ...
    ax[0,0].set_title("$X$")
    ax[0,1].set_title("$X_{masked}$")
    ax[0,2].set_title("$y$")
    if y_hat is not None:
       ax[0,3].set_title("$\hat{y}$")
